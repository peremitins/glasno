...
// Смена интервьюера в кабинете: выбранное лицо задаёт и внешность (фото),
// и тон ИИ (мягкий/нейтральный/строгий) — меняется на лету.
export default defineApiHandler(async (event) => {
  ...
  const input = await readDto(event, UpdateInterviewerRequestDto);
