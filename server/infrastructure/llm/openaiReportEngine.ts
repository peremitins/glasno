...
export function buildInstruction(): string {
  return [
    ...
    'Не выдумывай факты, цифры, названия компаний, сроки, метрики и результаты. Используй только данные из вакансии, резюме и ответа кандидата.',
    'Если для сильного ответа нужна конкретика, которой нет в материалах, вставь нейтральный плейсхолдер в квадратных скобках: [подставьте реальный результат], [укажите срок], [добавьте метрику].',
