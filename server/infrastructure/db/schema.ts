...
// Не удалять и не переименовывать поля без миграционного
...
// Учёт личностей, израсходовавших бесплатную попытку интервью (попытка
// расходуется созданием сессии, а не готовым отчётом). Запись создаётся при
// готовом отчёте, при линковке анонимных сессий к аккаунту и при удалении
// аккаунта с хотя бы одной сессией; не удаляется вместе с аккаунтом — это
// не даёт получить новую бесплатную попытку после удаления или перелогина.
// completedAt — момент фиксации попытки.
