...
- Для генерации, перевода, проверки, классификации и дедупликации контента репозитория используй только активную подписочную сессию Claude Code.
- Запрещено создавать служебные скрипты с Anthropic/OpenAI SDK, использовать `ANTHROPIC_API_KEY`, OpenAI-ключи приложения или relay-секреты и запускать через них платные batch-запросы.
- Ключи AI-провайдеров предназначены только для пользовательских runtime-функций приложения. Если задачу нельзя выполнить средствами подписки Claude Code, остановись и сообщи об ограничении.
