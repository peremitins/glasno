...
async function explainManualSelection() {
  ...
  try {
    manualExplanation.value = await explainTerm({
...
onMounted(() => {
  ...
  if (props.manualSelection) addManualSelectionListeners();
});
...
<template>
  <span
    ...
    class="text-with-interview-terms text-with-interview-terms--manual"
